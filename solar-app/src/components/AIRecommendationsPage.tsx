...
const CATEGORY_META = {
  weather:      { icon: '⛅', label: 'Weather Alert',    bg: 'bg-sky-50',    border: 'border-sky-200',    tag: 'bg-sky-100 text-sky-700' },
  equipment:    { icon: '🔧', label: 'Equipment',        bg: 'bg-red-50',    border: 'border-red-200',    tag: 'bg-red-100 text-red-700' },
  commercial:   { icon: '📊', label: 'Commercial',       bg: 'bg-purple-50', border: 'border-purple-200', tag: 'bg-purple-100 text-purple-700' },
  risk:         { icon: '🚨', label: 'Risk Alert',       bg: 'bg-orange-50', border: 'border-orange-200', tag: 'bg-orange-100 text-orange-700' },
  data_quality: { icon: '📋', label: 'Data Quality',     bg: 'bg-amber-50',  border: 'border-amber-200',  tag: 'bg-amber-100 text-amber-700' },
};
...
export const AIRecommendationsPage: React.FC = () => {
  ...
  if (!intervalRows.length) return <EmptyState icon="🤖" message="No data loaded" sub="Upload data to generate AI recommendations." />;
              ...
              <div className="flex-1">
                ...
                {rec.impact && (
                  <div className="flex items-center gap-2 text-xs text-slate-500 bg-white/60 rounded-lg px-3 py-2 mb-2">
                    <span>💰</span> <strong>Impact:</strong> {rec.impact}
                  </div>
                )}
                {rec.action && (
