...
export const OverviewPage: React.FC = () => {
  ...
  if (!intervalRows.length) return <EmptyState icon="📊" message="No data loaded yet" sub="Go to the Upload tab to load your dataset." />;
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        <KPICard icon="⚡" label="Total Actual Generation" value={fmtMWh(totalActual)} color="amber" tooltip="Sum of all actual metered energy in MWh." />
        <KPICard icon="🎯" label="AI Forecast Accuracy" value={accuracy !== null ? fmtPct(accuracy) : undefined} missing={!hasForecast ? 'ai_forecast_energy_mwh' : undefined} color="green" tooltip="100 − MAPE of forecast vs actual." />
        <KPICard icon="📅" label="Scheduled Energy" value={hasScheduled ? fmtMWh(totalScheduled) : undefined} missing={!hasScheduled ? 'scheduled_energy_mwh' : undefined} color="blue" />
        <KPICard icon="💰" label="Total Gross Revenue" value={hasTariff ? fmtINR(totalGross, true) : undefined} missing={!hasTariff ? 'tariff_inr_per_kwh' : undefined} color="teal" />
        <KPICard icon="⚠️" label="Deviation Penalty" value={hasScheduled ? fmtINR(totalPenalty, true) : undefined} missing={!hasScheduled ? 'scheduled_energy_mwh' : undefined} color="red" />
        <KPICard icon="🏆" label="Net Revenue" value={hasTariff ? fmtINR(totalNet, true) : undefined} missing={!hasTariff ? 'tariff_inr_per_kwh' : undefined} color="green" />
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard icon="🛡️" label="Penalty Avoided by AI" value={penaltyAvoided !== null ? fmtINR(penaltyAvoided, true) : '—'} sub={legacyAcc !== null ? `Legacy accuracy: ${fmtPct(legacyAcc)}` : undefined} color="green" tooltip="Estimated penalty reduction vs legacy forecasting." />
        <KPICard icon="📊" label="Data Quality Score" value={`${dqScore}/100`} color={dqScore >= 80 ? 'green' : dqScore >= 60 ? 'amber' : 'red'} />
        <KPICard icon="⛔" label="Edge Cases Detected" value={edges.length.toString()} sub={`${edges.filter(e => e.severity === 'critical').length} critical`} color={edges.length ? 'red' : 'green'} />
        <KPICard icon="🔧" label="Asset Faults" value={faults ? faults.toString() : assetRows.length ? '0' : undefined} missing={!assetRows.length ? 'asset_status' : undefined} color={faults ? 'red' : 'green'} />
      </div>
      ...
      {penaltyData.length > 0 && hasTariff && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <div className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">
            <span className="w-7 h-7 rounded-lg bg-red-50 flex items-center justify-center text-base">💸</span>
