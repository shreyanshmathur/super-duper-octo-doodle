  ...
  return (
    <div className="space-y-6 fade-in">
        ...
        <div className="flex items-center justify-between gap-6 flex-wrap">
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            <div className="text-xs text-slate-500">
              {score >= 80 ? '✅ Good quality — safe for SLDC submission and settlement.' : score >= 60 ? '⚠️ Moderate quality — review issues before submission.' : '❌ Poor quality — do not submit for settlement without fixing critical issues.'}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Checklist */}
        <Card title="Quality Checklist" icon="✅" iconBg="bg-green-50">
          <div className="space-y-2">
            {checklist.map(item => (
              <div key={item.label} className="flex items-center gap-3 py-1.5 border-b border-slate-50 last:border-0">
                <span className={`text-base ${item.pass ? 'text-green-500' : 'text-red-500'}`}>{item.pass ? '✅' : '❌'}</span>
        ...
        {/* Stats */}
        <Card title="Dataset Statistics" icon="📊" iconBg="bg-blue-50">
      ...
      {/* Issues */}
      {issues.length > 0 && (
        <Card title="Detected Issues" icon="⚠️" iconBg="bg-amber-50">
              ...
              <div key={i} className={`rounded-lg p-4 border ${issue.severity === 'critical' ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-blue-700 font-semibold">💡 Fix: {issue.suggestedFix}</div>
                    {issue.revenueImpact !== undefined && issue.revenueImpact > 0 && (
                      <div className="text-xs text-red-600 font-semibold mt-1">💸 Revenue impact: ~{fmtINR(issue.revenueImpact, true)}</div>
                    )}
                  </div>
                </div>
              </div>
