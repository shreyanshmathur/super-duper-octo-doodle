...
import { ShieldAlert, AlertTriangle, Info, CheckCircle2, Lightbulb } from 'lucide-react';
...
export const EdgeCasePage: React.FC = () => {
  ...
  if (!intervalRows.length) return <EmptyState icon={<ShieldAlert size={48} />} message="No data loaded" />;
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {Object.entries(grouped).map(([type, cases]) => {
        const sev = cases[0].severity;
        const icon   = sev === 'critical' ? <ShieldAlert size={15} className="text-red-500" />
                     : sev === 'warning'  ? <AlertTriangle size={15} className="text-amber-500" />
                     : <Info size={15} className="text-blue-500" />;
        const iconBg = sev === 'critical' ? 'bg-red-50' : sev === 'warning' ? 'bg-amber-50' : 'bg-blue-50';
        return (
          <Card key={type} title={`${type} (${cases.length})`} icon={icon} iconBg={iconBg}>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-1 text-xs text-blue-700 font-semibold">
                        <Lightbulb size={11} className="shrink-0" /> {e.recommendation}
                      </div>
